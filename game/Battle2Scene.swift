...
class Battle2Scene: SKScene, SKPhysicsContactDelegate {
    ...
    override func didMoveToView(view: SKView) {
        ...
        button = self.childNodeWithName("button_battle2") as! SKSpriteNode
        button2 = self.childNodeWithName("button2_battle2") as! SKSpriteNode
        button2p = self.childNodeWithName("button2p_battle2") as! SKSpriteNode
        button3 = self.childNodeWithName("button3_battle2") as! SKSpriteNode
        button3p = self.childNodeWithName("button3p_battle2") as! SKSpriteNode
        diglett = self.childNodeWithName("diglett_battle2") as! SKSpriteNode
        charmander = self.childNodeWithName("charmander_battle2") as! SKSpriteNode
        battle_wall_left = self.childNodeWithName("battle2_wall_left") as! SKSpriteNode
        battle_wall_right = self.childNodeWithName("battle2_wall_right") as! SKSpriteNode
        
        for i in 1 ... text_count {
            let text = self.childNodeWithName(String(format: "text_battle2_%d", i))
            text!.zPosition = 10
            if text_array == nil {
                text_array = [text!]
            } else {
        ...
        for i in 1 ... health_count {
            let health = self.childNodeWithName(String(format: "health%d_2", i))
            if health_array == nil {
                health_array = [health!]
            } else {
        ...
        for i in 1 ... enemy_health_count {
            let enemy_health = self.childNodeWithName(String(format: "health%de_2", i))
    ...
    func charmander_move(round: uint) {
        ...
        if round == 1 {
            ...
            timer2 = NSTimer(timeInterval: 0.5, target: self, selector: #selector(charmander_move2), userInfo: nil, repeats: true)
            NSRunLoop.currentRunLoop().addTimer(timer2, forMode: NSRunLoopCommonModes)
        }
    }
    
    func charmander_move2() {
        charmander.runAction(SKAction.moveByX(diglett.position.x - charmander.position.x, y: 0, duration: 0.5))
